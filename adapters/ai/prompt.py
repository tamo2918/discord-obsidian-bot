...
**既存ファイルがある場合**: 既存のfrontmatterをそのまま維持し、tagsのみ追加可能。新しいフィールドは追加しない。

**新規作成の場合のみ**、以下の形式でfrontmatterを付ける:
...
**重要**: source, author, status, aliasesなどのフィールドは使用しない
...
- **既存のfrontmatterをそのまま維持する**（type, created, processed, tagsのフィールド構成を変えない）
- tagsには内容に合ったタグを追加してよい
- **新しいフィールド（source, author, status, aliasesなど）を追加しない**
- 既存の `[[内部リンク]]` を壊さない
- `## メモ` セクションの下に新しいメモを追加する
